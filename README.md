...
Скрипт автоматически решает **29 типов заданий**:

| Солвер | Тип задания |
|--------|-------------|
| `TableFillSolver` | Заполнение таблиц по уравнению (y = mx + b) |
| `GraphLineSolver` | Построение графика линии по уравнению (y = mx + b) |
| `PlotPointsSolver` | Построение точек на графике по координатам или уравнению |
| `SelectEquationSolver` | Выбор уравнения по точкам на графике |
| `SelectConstantSolver` | Выбор коэффициента пропорциональности из уравнения |
| `SelectMatchInequalitySolver` | Выбор неравенства, соответствующего числовой прямой |
| `SelectAllMatchSolver` | Выбор всех выражений, равных заданному значению |
...
| `SelectFactorsSolver` | Выбор делителей числа из списка |
| `VisualLCMSolver` | Выбор НОК с визуальными блок-диаграммами |
| `LeastCommonMultipleSolver` | Выбор НОК с текстовыми числами |
| `VisualGCFSolver` | Поиск НОД с визуальными блок-диаграммами |
| `GreatestCommonFactorSolver` | Поиск НОД с текстовыми числами |
| `SelectEquivalentFractionSolver` | Выбор эквивалентной дроби |
| `FractionToDecimalChoiceSolver` | Преобразование дроби в десятичное число |
...
| `SolveForXSolver` | Решение уравнения для X с вариантами ответа |
| `EquationBlankSolver` | Уравнения с пропуском и выбором |
| `RatioChoiceSolver` | Выбор соотношения из вариантов |
...
│   └── PieChartParser.ts
│
├── solvers/            # Солверы заданий (29 штук)
...
### v1.0.21 (2026-01-29)
- **Новый тип задания:** Построение графика линии по уравнению (Graph the Line)
  - Задания вида "Graph the line" с уравнением типа `y = x` или `y = 2x + 3`
  - График содержит две перетаскиваемые точки, которые нужно расположить на линии
  - Статические точки показывают правильное положение линии
  - Алгоритм:
    * Извлекает уравнение из `INPUT_VARIABLES` в iframe (m и b) или из KaTeX аннотаций
    * Парсит линейное уравнение вида `y = mx` или `y = mx + b`
    * Вычисляет координаты двух точек на линии (по умолчанию x=1 и x=3)
    * Перемещает перетаскиваемые точки через `updatePosition` метода компонента DraggablePoint
    * Поддерживает несколько методов доступа к точкам: через `diagram.components`, `OUTPUT_VARIABLES`, или прямое обновление
  - Примеры решения:
    * Уравнение `y = x`: перемещает точки в (1, 1) и (3, 3)
    * Уравнение `y = 2x + 1`: перемещает точки в (1, 3) и (3, 7)
  - Поддерживает уравнения с отрицательными коэффициентами и константами
  - Регистрация: **ПЕРЕД** `InteractiveSliderSolver` (более высокий приоритет)
  - Тесты: готов к тестированию

### v1.0.20 (2026-01-29)
- **Новый тип задания:** Заполнение таблиц по уравнению (Table Fill)
  - Задания вида "Complete the table" с уравнением типа `y = 2x` или `y = 2x + 3`
  - Таблица содержит значения `x` и пустые ячейки для `y`
  - Доступные токены для заполнения предоставляются в задании
  - Алгоритм:
    * Извлекает уравнение из KaTeX аннотаций
    * Парсит линейное уравнение вида `y = mx` или `y = mx + b`
    * Извлекает данные таблицы из `INPUT_VARIABLES` в iframe
    * Вычисляет недостающие значения `y` на основе `x` и уравнения
    * Заполняет ячейки таблицы через `setCellValue` метода компонента Table
  - Примеры решения:
    * Уравнение `y = 2x`, таблица: `x=-4 → y=?`, `x=-2 → y=?`, `x=1 → y=?`, `x=3 → y=?`
    * Вычисляет: `y(-4) = -8`, `y(-2) = -4`, `y(1) = 2`, `y(3) = 6`
    * Находит соответствующие токены и заполняет таблицу ✓
  - Поддерживает уравнения с отрицательными коэффициентами и константами
  - Регистрация: **ПЕРЕД** `InteractiveSliderSolver` (более высокий приоритет)
  - Тесты: готов к тестированию

### v1.0.19 (2026-01-29)
- **Исправлено:** ExpressionBuildSolver не мог решить задания с десятичными числами
  - Проблема: токены вида `0.8`, `0.1`, `-0.7` не парсились (использовался `parseInt` вместо `parseFloat`)
  - Исправлено: обновлены регулярные выражения для поддержки десятичных чисел (`-?\d+(?:\.\d+)?`)
  - Добавлена поддержка десятичных чисел в `renderNumber()`, обычных числах и строковых токенах
  - Добавлено сравнение с допуском для чисел с плавающей точкой (`isEqualWithTolerance`)
  - Пример: уравнение `0=\duoblank{3}` с токенами `[0.8, 0.1, -0.7, -0.1, +, -0.65]`
  - Теперь правильно находит решение: `0.1 + (-0.1) = 0` ✓
- **Исправлено:** TypeAnswerSolver неправильно определял уравнения как неравенства
  - Проблема: уравнение `10+\left( -4 \right)=\duoblank{1}` определялось как неравенство из-за символов `>` в `\left` и `\right`
  - Исправлено: улучшена логика определения неравенств с учетом LaTeX команд `\left` и `\right`
  - Если уравнение содержит `=` и `\left`/`\right`, но нет явных операторов неравенства (`>=`, `<=`, `\ge`, `\le`), то это уравнение
  - Пример: `10+\left( -4 \right)=\duoblank{1}` → правильно решает как `10 + (-4) = 6` вместо неправильного `10/1` ✓

### v1.0.18 (2026-01-29)
- **Улучшение PatternTableSolver:** Теперь правильно обрабатывает варианты ответов вида `X=9` или `X=15`
  - Исправлена проблема с извлечением числовых значений из уравнений
  - Регулярное выражение извлекает число из строк вида "X=9", "X = 9", "9.5", "-5" и т.д.
  - Пример: задание "Follow the pattern" с таблицей `12-1=X → X=11`, `12-2=X → X=10`, `12-3=X → ?`
  - Вычисляет `12-3=9` и правильно находит вариант `X=9` среди `X=9` и `X=15` ✓
  - Все 207 тестов проходят

### v1.0.18 (2026-01-29)
- **Улучшение PatternTableSolver:** Теперь правильно обрабатывает варианты ответов в формате `X=9` или `X=15`
  - Исправлена проблема с извлечением чисел из уравнений вида `X=9`
  - Добавлена поддержка форматов: `"9"`, `"X=9"`, `"X = 9"`, `"9.5"`, `"-5"` и т.д.
  - Регулярное выражение `/(-?\d+\.?\d*)/` извлекает число из любого формата
  - Примеры решения:
    * `12-3=X` → вычисляет `9` → находит `X=9` из вариантов `[X=9, X=15]` ✓
    * `10-3=X` → вычисляет `7` → находит `X=7` из вариантов `[X=7, X=13]` ✓
  - Тесты: 3 теста для PatternTableSolver (всего 207)
